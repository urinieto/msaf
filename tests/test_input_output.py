...
def test_read_hier_references():
    one_jams = os.path.join(
        "..", "datasets", "Sargon", "references", "01-Sargon-Mindless.jams"
    )
    three_jams = os.path.join(
        "..", "datasets", "SALAMI", "references", "SALAMI_200.jams"
    )
    ...
    # One level file
    hier_bounds, hier_labels, hier_levels = msaf.io.read_hier_references(one_jams)
    assert len(hier_bounds) == len(hier_labels) and len(hier_labels) == len(hier_levels)
    assert len(hier_levels) == 1

    # Three level file
    hier_bounds, hier_labels, hier_levels = msaf.io.read_hier_references(three_jams)
    assert len(hier_bounds) == len(hier_labels) and len(hier_labels) == len(hier_levels)
