...
def test_add_var():
    ...
    val = 10
    AddConfigVar("test.new_var", "Test Variable only for unit testing", IntParam(val))
...
def test_bool_var():
    ...
    AddConfigVar(
        "test.my_new_bool", "Test bool variable only for testing", BoolParam(True)
    )
...
def test_empty_config_val():
    ...
    conf = msaf.configparser.parse_config_string(MSAF_FLAGS, issue_warnings=True)
...
def test_fetch_too_many_sections():
    ...
    with raises(KeyError):
        msaf.configparser.fetch_val_for_key("caca.merda.merdeta", delete_key=False)
...
def test_add_filter_config_var():
    """Tests adding a var that already exists."""
    configparam = EnumStr(
        ".wav", ".mp3", ".aif", convert=lambda x: x.replace("wav", "aif")
    )
