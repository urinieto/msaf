...
def compute_cbm(autosimilarity, min_size = 1, max_size = 32, penalty_weight = 1, penalty_func = "modulo8", bands_number = None):
    ...
    max_conv_eight = np.amax(corrleation_entire_matrix_computation(autosimilarity, kernels))
    ...
    for current_idx in range(1, len(autosimilarity)): # Parse all indexes of the autosimilarity
        for possible_start_idx in possible_segment_start(current_idx, min_size = min_size, max_size = max_size):
            if possible_start_idx < 0:
                raise ValueError(f"Invalid value of start index: {possible_start_idx}, shouldn't happen.") from None
                
            # Convolutionnal cost between the possible start of the segment and the current index (entire segment)
            conv_cost = conrrelation_cost(autosimilarity[possible_start_idx:current_idx,possible_start_idx:current_idx], kernels)
    ...
    while precedent_frontier > 0:
        ...
        if precedent_frontier == None:
            raise ValueError("Well... The dynamic programming algorithm took an impossible path, so it failed. Understand why.") from None
...
def conrrelation_cost(cropped_autosimilarity, kernels):
    ...
    return np.sum(np.multiply(kern,cropped_autosimilarity)) / p**2 # May be useful to modify the normalization.
...
def corrleation_entire_matrix_computation(autosimilarity_array, kernels, kernel_size = 8):
    ...
    for i in range(kernel_size, len(autosimilarity_array)):
        cost[i] = conrrelation_cost(autosimilarity_array[i - kernel_size:i,i - kernel_size:i], kernels)
    return cost
...
def possible_segment_start(idx, min_size = 1, max_size = None):
    ...
    if min_size < 1: # No segment should be allowed to be 0 size
        raise ValueError(f"Invalid minimal size: {min_size} (No segment should be allowed to be 0 or negative size).")
        #min_size = 1
    if max_size == None:
