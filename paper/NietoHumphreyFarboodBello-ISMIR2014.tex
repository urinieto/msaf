...
\section{Testing the Subjectivity Hypothesis}

One of the most fundamental problems of music segmentation is the lack of a formal and standard definition of \textit{section} or \textit{boundary}. 
The difficulty of agreeing upon a single definition becomes apparent when qualitatively investigating the audio of the hardest tracks: some boundaries seem rather arbitrary, leaving the final decision to the annotator. 
...
This shows how the metric MGP is qualitatively working as expected.
...
We analyzed all the reported tracks and qualitatively decided that all of them could fall into five main categories:
...
\begin{itemize}
  ...
  \item
    \textbf{Instrumentation}: Various tags that had to do with instrument peculiarities were also reported. Examples: ``vocals only'', ``instrumental''.

  \item
    \textbf{Sytle}: The final group is the one related with the music genre of the piece, which includes, among other qualities, the speed and the homogeneity of the track. Examples: ``fast'', ``jazz''.
\end{itemize}

In Figure \ref{fig:difficult-tags-type} the frequency of each tag group is shown, and it can be seen that two main groups dominate the histogram: style and form.
In terms of style, we hypothesize that there will be a strong biased depending on the preferred music genre of the subjects.
For example, there was considerable agreement of finding the jazz pieces difficult to segment, while all the subjects were classically trained.
...
Now that we know hat are the main qualities that make some tracks difficult to segment from a human perspective we aim to explore the amount of agreement across subjects in order to better understand the amount of subjectivity for this task.
...
At a first glance we already see how the number of annotators reporting tags is almost uniformly distributed across the dataset, showing the lack of subjective agreement 
...
We will go back to these tracks when performing the analysis of the boundaries.
...
So far, we have already seen that subjectivity might play an important role when evaluating music boundaries, at least from the qualitative perception of subjects.
We will now analyze the actual boundaries that subjects annotated to evaluate the degree of disagreement.
