...
This task aims to retrieve all the boundaries of the segments (or sections) from a music audio signal, in order to better understand the structure of the piece \cite{Paulus2010}.
...
It can be particularly challenging due to (i) the difficulty of standardizing the definition of \emph{boundary} or \emph{segment} \cite{Smith2011}, and (ii) because of the apparent role that subjectivity plays on this task \cite{Bruderer2006a}.
%This task is often associated with the labeling of these segments based on their acoustic similarity, but in this work we will only focus on the boundary identification problem.
...
%This leads to multiple interpretations of the identification and placement of the boundaries, which suggests that subjectivity might play an important role \cite{Bruderer2006a}.
% Definition problem, subjectivity in the oebserver

%TODO Eric: Needs context. Hard to define task, and the criteria that consitutes a boundary is subjective.
% So far, we have been able to define these boundaries:
Nevertheless, multiple automatic techniques have been published, reporting results that indicate that we are still far from solving this task.
...
% Performance is generally still quite low, and we try to understand why it is so bad.
% Cite holzapfel and serra, say that we get tracks to get annotations.
% Serra, might be advantageous to have multiple annotations.
Using a similar approach as in \cite{Grosche2010}, where the authors investigate the inherent problems of beat tracking, we decide to explore some of the most arduous tracks in terms of segmentation and perform error analysis on them.
We use our structural framework to analyze a large dataset of over 2,000 human annotated tracks to identify the worst and best performing pieces in Section \ref{sec:methodology}.
This approach has proven to be reliable when identifying some of the biggest challenges in beat tracking \cite{Holzapfel2012}. % shown that this approach could help identify various audio cues that might help automatic beat tracking approaches to perform better \cite{Holzapfel2012}.
%Moreover, studies have shown that by exploring the most challenging tracks of a large dataset, we can identify audio cues that might help us improve automatic approaches
Moreover, it has been shown that not even humans agree when they manually undertake the task of boundary detection \cite{Serra2013}.
This puts into question the notion of ground truth with a single human annotation, which motivates us to collect multiple annotations for a subset containing some of the most challenging tracks, a process that is described in Section \ref{sec:multiple-humans}.
...
In Section \ref{sec:data-anal} we analyze the agreement of these new annotations and show how they yield different results when used as ground truth to evaluate the algorithms of our structural framework, which illustrates the weakness of the idea of ground truth for this task.
Furthermore, in the same section, we show how a merged version of the annotated boundaries can result in a more stable ground truth, therefore alleviating some of the problems that arise from subjectivity.
...
%Furthermore, in the same section, we aggregate the multiple boundaries into a single weighted annotation, and show how it becomes a much more stable and reliable ground truth.
...
In this section we detail the large collection of human annotated boundaries used in our project; explain the algorithms to be run in in the dataset; describe the evaluation metrics to rate and rank the algorithms; and introduce a music structural analysis framework that will facilitate the exploration of the best and worst performing tracks, among other desired qualities.
...
\section{Acquiring Multiple Human Annotations}\label{sec:multiple-humans}
...
%This suggests that short musical pieces might be easier for machines to identify their boundaries, while human annotators may have problem with such short The fact that 
