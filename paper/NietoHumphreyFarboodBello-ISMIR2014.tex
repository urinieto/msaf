...
In this work we aim to identify the most challenging tracks of our dataset (from a machine perspective) in order to setup an experiment to test the amount of boundaries agreement among multiple subjects.
...
We run our algorithms on our \textbf{BIG} dataset and sort their tracks based on their Mean Ground-truth Performance (MGP) using the hit rate at 3 seconds described in \ref{subsec:hitrate}.
...
In Figure \ref{fig:difficult-tags-type} the frequency of each tag group is shown, and it can be seen that two main clusters dominate the plot: style and form.
...
On the other hand, the most reported tag of all the collection was the variability of a track, which falls in the form group (in red in the figure).
...
We use the ranking stability measure in order to quantize this amount of variation, and we the results are shown in table \ref{tab:stability-multipleGT}.
TODO: Table!
This illustrates that the notion of ground truth is remarkably weak for boundary estimation.
...
In order to improve this problem, we aim to use multiple annotations as a weighted single ground truth in order to produce more stable results.
...
In this case, as in the standard evaluation, a hit is found when an estimated boundary is within a 3 seconds window from the closest reference one.

Once we have the weighted $H$ computed, we obtain the precision and the recall values by normalizing using the weights of the estimation and reference, respectively:
...
The F-meauser between the precision and the recall is computed as usual.
